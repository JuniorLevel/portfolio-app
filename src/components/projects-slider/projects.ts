import 'interfaces/interfaces.ts';

export const data: IProject[] = [
  {
    id: 0,
    name: 'Qpick store telegram bot',
    description: 'Телеграм бот c мини веб приложением Qpick Store',
    stack: 'ReactJS + Typescript / NodeJS + PostgreSQL',
    picture: '/images/projects/qpick-tg.jpg',
    url: 'https://t.me/qpick_store_bot',
  },
  {
    id: 1,
    name: 'Cloud App',
    description: 'Облачное хранилище',
    stack: 'ReactJS + Typescript + Zustand / NodeJS + ExpressJS + MongoDB',
    picture: '/images/projects/cloud-app.png',
    url: 'https://cloud-app.netlify.app/',
  },
  {
    id: 2,
    name: 'Qpick Store App E-commerce',
    description: 'Интернет магазин на основе API Platzi Fake Store',
    stack: 'ReactJS + TypeScript + Redux Toolkit',
    picture: '/images/projects/qpick-store.png',
    url: 'https://zesty-sprite-912bbd.netlify.app/',
  },
  {
    id: 3,
    name: 'Landing Prime Car',
    description: 'Одностраничный сайт по продаже машин',
    stack: 'HTML (BEM naming) / CSS / JavaScript',
    picture: '/images/projects/prime-car.png',
    url: 'https://boisterous-cranachan-8377fa.netlify.app/',
  },
];
